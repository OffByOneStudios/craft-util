// From: https://testbit.eu/cpp11-signal-system-performance/
...
namespace Detail
{
	...
	template<typename, typename> struct CollectorInvocation; // MJB: was fwd declared class
	...
	private:
		...
		struct SignalLink {
			...
			void unlink()
			{
				// MJB: NULL -> nullptr fixes MSVC 2015's poor SFINAE
				function = nullptr;
				if (next)
					next->prev = prev;
				if (prev)
...
template <typename SignalSignature, class Collector = Detail::CollectorDefault<typename std::function<SignalSignature>::result_type> >
	struct Signal /*final*/ :
		Detail::ProtoSignal<SignalSignature, Collector>
{
